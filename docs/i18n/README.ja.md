...
Guruguru Codechan は、VS Code の中にドッキング可能な
Codeちゃん相棒ビューを追加する拡張機能です。
Codeちゃんはまばたきし、
静かにあなたの作業を見守り、マウスが近づくと視線で追います。

作業に疲れたら、少しだけCodeちゃんと遊んで癒やされてみましょう。
...
- 設定から位置、拡縮、視線固定、追従範囲、追従速度、口パク同期、自動まばたきを調整できます。
- 自分で用意した 150 フレームのぐるぐる風キャラクター素材を読み込めます。
...
VS Code 起動後に自動で開きたい場合は、設定の `Guruguru Codechan: Open On Startup` を有効にしてください。
最初に開くキャラクターは `Guruguru Codechan: Default Character` で指定できます。
`Default` は前回のキャラクターを維持し、`Random` はビューを開くたびに抽選します。
...
拡張機能はフレーム名を検証し、欠けたフレームや形式の混在を拒否します。
512px より大きいフレームは、保存時に長辺 512px に縮小されます。
`Default` と `Random` は予約名のため、読み込むキャラクター名には使えません。
...
- 口パク: 設定で有効にした場合、キーボード入力に反応して口パクします。将来的には TTS との連携にも使える `mouthLevel` チャンネルを残しています。
...
一度マウスでCodeちゃんの視線を誘導してから、エディタに戻ってみてください。
Codeちゃんは最後にマウスがビューから出た方向を使ってエディタの位置を推測します。

#### > Codeちゃんに固定方向だけを見てほしい

設定ボタンを押し、拡張ビュー内の空白部分をクリックして、
表示された `Lock Gaze` を選んでください。
...
このプロジェクトには、トマリちゃん関連の素材は含まれていません。

プログラムコードは MIT ライセンスです。
付属サンプル素材の Codeちゃん は Hitsuki がデザインした同人キャラクターです。
